...
if ($pedidoId > 0) {
    $stmtPedido = $conexion->prepare('SELECT id, estado, nombre, direccion, telefono, metodo FROM pedidos WHERE id = ? LIMIT 1');
...
$resultadoProductos = $conexion->query('SELECT id, nombre, descripcion, precio, stock FROM productos ORDER BY id DESC');
...
<header class="barra-cliente">
    ...
    <nav class="nav-cliente">
        ...
        <a href="consultar_pedido.php">Mis pedidos</a>
...
<main class="contenedor-principal">
    ...
    <section class="hero-cliente">
        <div class="hero-texto">
            <p class="etiqueta">ORGANIC ZONE</p>
            <h1>Disfruta algo<br><span>delicioso.</span></h1>
            <p class="hero-descripcion">
                Elige tus productos favoritos y arma tu pedido de forma rápida y sencilla.
            </p>
            ...
            <?php if ($pedidoId > 0): ?>
                <div class="pedido-activo">
                    ...
                    <small><?= htmlspecialchars($pedidoEstado) ?></small>
                </div>
            <?php else: ?>
                <button type="button" class="boton-principal" id="abrirPedido">
                    Generar pedido
                </button>
            <?php endif; ?>
        </div>
    ...
    <section class="seccion-productos" id="productos">
        <div class="titulo-seccion">
            <div>
                <p>ELIGE TUS FAVORITOS</p>
                <h2>Nuestros productos</h2>
            </div>

            <div class="estado-compra <?= $pedidoId > 0 ? 'activo' : '' ?>" id="estadoCompra">
                <?php if ($pedidoId > 0): ?>
                    <span class="punto"></span>
                    Pedido #<?= $pedidoId ?> listo para agregar productos
                <?php else: ?>
                ...
                <?php while ($producto = $resultadoProductos->fetch_assoc()): ?>
                    ...
                    <article class="producto-card">
                        <div class="producto-imagen-wrap">
                            <img src="<?= htmlspecialchars($imagen) ?>" alt="<?= htmlspecialchars($producto['nombre']) ?>">
                            <?php if ($sinStock): ?>
                                <span class="etiqueta-stock sin-stock">Sin stock</span>
                            <?php else: ?>
                                <span class="etiqueta-stock">Stock: <?= $stockProducto ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="producto-info">
                            ...
                            <div class="producto-pie">
                                <strong>Bs. <?= number_format((float)$producto['precio'], 0, ',', '.') ?></strong>

                                <button
                                    type="button"
                                    class="boton-agregar"
                                    data-producto-id="<?= $idProducto ?>"
                                    <?= ($pedidoId <= 0 || $pedidoConfirmado || $sinStock) ? 'disabled' : '' ?>
                                >
                                    Agregar
                                </button>
                            </div>
    ...
    <section class="seccion-informacion">
        <div class="info-card verde">
            ...
            <p>Ingresa tus datos una sola vez para comenzar tu compra.</p>
        </div>
        <div class="info-card crema">
            <span>02</span>
            <h3>Agrega productos</h3>
            <p>El carrito se actualiza automáticamente sin recargar la página.</p>
        </div>
        <div class="info-card cafe">
            <span>03</span>
            <h3>Confirma y consulta</h3>
            <p>Recibe tu comprobante y revisa el estado de tu pedido.</p>
        </div>
    </section>
...
<div class="modal-overlay" id="modalPedido">
    <div class="modal-caja">
        ...
        <p class="modal-etiqueta">NUEVO PEDIDO</p>
        <h2>Comencemos tu compra</h2>
        ...
        <form id="formPedido">
            ...
            <button type="submit" class="boton-principal ancho-completo">
                Crear pedido
            </button>
        </form>

        <p class="mensaje-form" id="mensajePedido"></p>
    </div>
</div>

<!-- CARRITO LATERAL -->
<div class="carrito-overlay" id="carritoOverlay"></div>
<aside class="carrito-panel" id="carritoPanel">
    <div class="carrito-header">
        <div>
            <p>MI PEDIDO</p>
            <h2>Carrito</h2>
        </div>
        <button type="button" class="modal-cerrar" id="cerrarCarrito">×</button>
    </div>
    ...
    <div class="carrito-footer">
        <div class="total-linea">
            <span>Total</span>
            <strong id="totalCarrito">Bs. 0</strong>
        </div>

        <button type="button" class="boton-principal ancho-completo" id="finalizarPedido" disabled>
            Finalizar pedido
        </button>
    </div>
</aside>
